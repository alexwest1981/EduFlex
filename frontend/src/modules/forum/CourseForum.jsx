...
const CourseForum = ({ courseId, currentUser }) => {
    ...
    const fetchThreads = async (catId, page = 0) => {
        setIsLoading(true);
        try {
            const response = await api.forum.getThreads(catId, page, 10);
            if (response.content) {
                // Paginerat svar
                ...
                // Fallback för gamla API-svar (om backend inte uppdaterats än)
                setThreads(response.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)));
                setTotalPages(1);
                setCurrentPage(0);
            } else {
                setThreads([]);
    ...
    const handleSelectCategory = async (cat) => {
        ...
        setView('threads');
        await fetchThreads(cat.id, 0);
    };

    const handlePageChange = (newPage) => {
        if (newPage >= 0 && newPage < totalPages) {
            fetchThreads(currentCategory.id, newPage);
        }
    };
                ...
                {totalPages > 1 && (
                    <div className="flex justify-center items-center gap-4 mt-6">
                        <button
                            ...
                            className="px-4 py-2 rounded-lg border bg-white hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed font-medium text-gray-700 transition-colors"
                        >
                            Föregående
                        </button>
