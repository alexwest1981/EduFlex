                            ...
                            <div key={index} onClick={() => setSelectedEvent(item)} className="p-4 flex items-center justify-between hover:bg-indigo-50/50 cursor-pointer transition-colors group">
                                <div className="flex items-center gap-4">
                                    <div className="bg-gray-50 border p-2 rounded-lg text-gray-600 font-bold text-center min-w-[60px] group-hover:bg-white group-hover:border-indigo-200 transition-colors">
                                        <div className="text-[10px] uppercase tracking-wide">{new Date(item.event.date).toLocaleDateString(i18n.language, {month:'short'})}</div>
                                    ...
                                    <div>
                                        ...
                                        <div className="text-xs text-gray-500 flex gap-2 items-center mt-1"><span className="bg-gray-100 px-2 py-0.5 rounded text-gray-600 font-medium">{item.event.type}</span><span>{new Date(item.event.date).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</span></div>
                                    </div>
                                </div>
            ...
            {selectedEvent && (
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center z-50 p-4" onClick={() => setSelectedEvent(null)}>
                    <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl animate-in zoom-in duration-200 border border-gray-200" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="flex items-center gap-2 text-gray-600 text-sm mb-6 bg-gray-50 p-3 rounded-lg border"><Calendar size={16} className="text-indigo-500"/><span className="font-medium capitalize">{new Date(selectedEvent.event.date).toLocaleDateString(i18n.language, {weekday:'long', day:'numeric', month:'long', hour:'2-digit', minute:'2-digit'})}</span></div>
