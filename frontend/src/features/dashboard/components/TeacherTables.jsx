                    ...
                    {ungradedSubmissions.map(sub => (
                        <tr key={sub.id} className="hover:bg-gray-50 dark:hover:bg-[#282a2c]/50">
                            ...
                            <td className="p-4 text-right">
                                <button onClick={() => navigate(`/course/${sub.assignment?.course?.id || sub.assignment?.courseId || ''}?tab=assignments`)} className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-xs font-bold hover:bg-indigo-700 shadow-sm">{t('teacher_tables.go_to_grading')}</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
