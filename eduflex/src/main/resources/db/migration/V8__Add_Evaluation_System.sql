...
CREATE TABLE evaluation_responses_v2 (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
-- Index for analytics
CREATE INDEX idx_eval_instance_course ON evaluation_instances(course_id);
CREATE INDEX idx_eval_response_instance ON evaluation_responses_v2(instance_id);

-- Seed Skolverket Default Template
INSERT INTO evaluation_templates (name, description, is_system_template, created_at) 
VALUES ('Skolverket Standardutvärdering', 'Standardmallen för kursutvärdering enligt Skolverkets riktlinjer.', true, CURRENT_TIMESTAMP);

INSERT INTO evaluation_template_questions (template_id, question_text, question_type, order_index) 
SELECT id, 'Hur väl stämde kursens innehåll med målen?', 'LIKERT', 0 FROM evaluation_templates WHERE name = 'Skolverket Standardutvärdering';

INSERT INTO evaluation_template_questions (template_id, question_text, question_type, order_index) 
SELECT id, 'Hur upplevde du de pedagogiska metoderna?', 'LIKERT', 1 FROM evaluation_templates WHERE name = 'Skolverket Standardutvärdering';

INSERT INTO evaluation_template_questions (template_id, question_text, question_type, order_index) 
SELECT id, 'Har du fått tillräckligt med stöd under kursen?', 'LIKERT', 2 FROM evaluation_templates WHERE name = 'Skolverket Standardutvärdering';

INSERT INTO evaluation_template_questions (template_id, question_text, question_type, order_index) 
SELECT id, 'Hur var arbetsbelastningen?', 'LIKERT', 3 FROM evaluation_templates WHERE name = 'Skolverket Standardutvärdering';

INSERT INTO evaluation_template_questions (template_id, question_text, question_type, order_index) 
SELECT id, 'Hur bra var studiematerialet?', 'LIKERT', 4 FROM evaluation_templates WHERE name = 'Skolverket Standardutvärdering';

INSERT INTO evaluation_template_questions (template_id, question_text, question_type, order_index) 
SELECT id, 'Vad var det bästa med kursen?', 'TEXT', 5 FROM evaluation_templates WHERE name = 'Skolverket Standardutvärdering';

INSERT INTO evaluation_template_questions (template_id, question_text, question_type, order_index) 
SELECT id, 'Vad kan förbättras?', 'TEXT', 6 FROM evaluation_templates WHERE name = 'Skolverket Standardutvärdering';

INSERT INTO evaluation_template_questions (template_id, question_text, question_type, order_index) 
SELECT id, 'Hur sannolikt är det att du skulle rekommendera denna kurs till en vän?', 'NPS', 7 FROM evaluation_templates WHERE name = 'Skolverket Standardutvärdering';
