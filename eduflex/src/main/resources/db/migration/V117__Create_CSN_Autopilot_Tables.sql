...
CREATE TABLE course_enrollment_details (
...
CREATE INDEX idx_ced_course_student ON course_enrollment_details (course_id, student_id);
CREATE INDEX idx_ced_status ON course_enrollment_details (status);
CREATE INDEX idx_cel_unreported ON csn_event_log (is_reported_to_csn) WHERE is_reported_to_csn = false;
