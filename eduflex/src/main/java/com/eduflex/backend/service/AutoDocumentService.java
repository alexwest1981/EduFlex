...
public class AutoDocumentService {
    ...
    public Document generateCourseCertificate(CourseResult result) {
        try {
            ...
            // Add QR code to PDF
            byte[] pdfWithQR = addQRCodeToPDF(pdfBytes, verificationCode);
    ...
    public byte[] generateConsolidatedGradeReport(Long studentId) throws DocumentException, IOException {
        ...
        // --- HEADER ---
        Font titleFont = new Font(Font.HELVETICA, 24, Font.BOLD, new Color(30, 41, 59));
        Paragraph title = new Paragraph("SAMLADE BETYG", titleFont);
     ...
     */
    private byte[] addQRCodeToPDF(byte[] pdfBytes, String verificationCode) 
        ...
        // Add QR code to first page (bottom-right corner)
        PdfContentByte canvas = stamper.getOverContent(1);
        Image qrPdfImage = Image.getInstance(qrImage, null);
        
        // Position in bottom-right corner with some margin
        float qrSize = 100; // Smaller than generation size for better placement
        ...
        qrPdfImage.setAbsolutePosition(pageWidth - qrSize - marginRight, marginBottom);
        
        // Add image to canvas (not document)
        try {
