...
| **Hög** | **SUSA-navet Integration (LLP v1)** | Live-koppling mot Skolverkets SusaNav REST-API. Admin kan hämta nationellt godkända YH-program via SUN-kod. Kurser skapas automatiskt med SUN-kod, LIA-flagga och modulstruktur. | ✅ **Klar** (v3.7.0) | Eliminerar manuell kursregistrering. EduFlex = Alvis-ersättare. |
| **Hög** | **Utbildningskatalog (/programs)** | Ny publik katalog-sida i sidomenyn där studenter, sökande och SYV kan bläddra bland nationellt importerade utbildningsprogram, filtrera på LIA och ansöka direkt. | ✅ **Klar** (v3.7.0) | LLP-transformation: från LMS till Utbildningsportal. |
...
| **Hög** | **Global Language Sync** | Full restaurering av språkstöd (9+ språk), AI-driven synk och 100% lokalisering av landningssida & modal. | ✅ **Klar** (v3.9.2) | Eliminera statiska filer och stödja internationell försäljning. |
...
| **AI Gateway** | Python (FastAPI) → Gemini API | ✅ Aktiv |
