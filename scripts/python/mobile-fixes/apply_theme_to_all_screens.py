...
def remove_inline_color_overrides(content):
    ...
    content = re.sub(r'\\[styles\\.(\\w+), \\{ color: \\'
'#[0-9A-Fa-f]{6}\\') \\}\\]', r'styles.\\\1', content)
    
    # Pattern 2: [styles.X, { backgroundColor: '#...' }] -> styles.X
    content = re.sub(r'\\[styles\\.(\\w+), \\{ backgroundColor: \\'
'#[0-9A-Fa-f]{6}\\') \\}\\]', r'styles.\\\1', content)
...
'#[0-9A-Fa-f]{6}\\') \\}\\]', r'styles.\\\1', content)
    ...
    content = re.sub(r'\\[styles\\.(\\w+), \\{ backgroundColor: \\'
'#[0-9A-Fa-f]{6}, borderColor: \\'
'#[0-9A-Fa-f]{6}\\') \\}\\]', r'styles.\\\1', content)
    
    # Pattern 5: [styles.X, { borderColor: '#...' }] -> styles.X
    content = re.sub(r'\\[styles\\.(\\w+), \\{ borderColor: \\'
'#[0-9A-Fa-f]{6}\\') \\}\\]', r'styles.\\\1', content)
...
def update_imports(content):
    ...
    content = re.sub(
        r"import \\{\\s*useTheme\\s*\\} from '.*?ThemeContext';\\nimport \\{\\s*getThemeColors\\s*\\} from '.*?themeStyles';",
...
def update_hook_usage(content):
    ...
    content = re.sub(
        r"const \\{\\s*currentTheme\\s*\\} = useTheme\(\);\\s*const colors = getThemeColors\(currentTheme\);",
